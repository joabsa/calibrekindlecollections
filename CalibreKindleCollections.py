...
The Kindle is a great reading device, unfortunately so far it is rather
painful to use it for a large collection of documents -  for example
when dealing with many research papers.

Uses the metadata.calibre file generated by Calibre.

As Amazon doesn't seem to support this functionality yet, this is a hack
which also requires that the Kindle be restarted to update the system.
This is quite annoying, but there doesn't seem to be another way until
Amazon releases its `Kindle Development Kit`. At which point this script
...
Note that if the script is saved in the root folder of the Kindle
(e.g. /Volumes/Kindle), the mount point is not necessary.

Please feel free to use and extend the script in any way you want. If you
already have a valued collection of collections, it might also be a good
idea to backup this file before you give this a try.
'''
...
def setup():
    '''
    In case the script is not run in the root folder of the Kindle device, the
    ...
    parser = argparse.ArgumentParser(description='Create collections for books\
 maintained by Calibre for the Kindle')
    parser.add_argument("--nt", "--notags", action="store_false",
                       dest="createTags", default=not noTags,
                       help="Do not use tags to create collections")
    parser.add_argument("--na", "--noauthors", action="store_false",
                       dest="createAuthors", default=not noAuthors,
                       help="Do not use authors to create collections")
    parser.add_argument("--ns", "--noseries", action="store_false",
                       dest="createSeries", default=not noSeries,
                       help="Do not use series to create collections")

    parser.add_argument("--et", "--excltags",dest="excludeTags",
                       default = excludeTags,
                       help="Exclude the comma separated tags when creating\
 collections")

    parser.add_argument("-m","--mnt", dest="mntPoint", default=".",
          help="Required if script is not run from the root folder of Kindle.")
    parser.add_argument("-v","--verbose", dest="verbose", default=False,
                       action="store_true", help="Show more information")
    parser.add_argument("-q","--quiet",dest="quiet",default=False,
                       action="store_true",help= "Show less information")
...
def updateCollections():
    '''
    Calibre data is traversed each book encountered is added to a collection
    with the name of each author and series (if defined).
    '''
...
if __name__ == '__main__':
    ...
    if not (os.path.exists(COLLECTIONS) and os.path.exists(CALIBRE)):
        log.error("Unknown path to Kindle mounting point. Please correct path \
(e.g. /Volumes/Kindle) on the command line")
        sys.exit()
